...
::: {.callout-note title="📚 MMRM 方法學的 robust citations"}
本章 MMRM pipeline 對齊以下 RCT 縱向資料分析的 methodological consensus：
- **MMRM > LOCF / ANOVA**：Mallinckrodt 2008[@mallinckrodt2008mmrm]、Mallinckrodt 2004 covariance structure[@mallinckrodt2004correlation]、Gueorguieva & Krystal 2004 *Move Over ANOVA*[@gueorguieva2004moveover]
- **MMRM bias / type I error**：Bell & Rabe 2020 *Trials*[@bell2020mmrm]
- **Missing data 處理 / MAR 假設 / FDA 觀點**：Little 2012 NRC report *NEJM*[@little2012nrcreport]、LaVange & Permutt 2016 regulatory perspective[@lavange2016fda]
- **ICH E9 (R1) estimands framework**：Mehrotra et al. 2016[@mehrotra2016estimands]
...
- **ANOVA**：把每個時間點當獨立 group，沒利用「同一隻眼睛」的 within-subject correlation——這正是 Gueorguieva & Krystal 2004 *Archives Gen Psychiatry* 〈Move Over ANOVA〉所主張要被 MMRM 取代的原因[@gueorguieva2004moveover]
- **MMRM**：明確 model 「同一隻眼睛在不同 visit 之間相關」[@mallinckrodt2008mmrm; @mallinckrodt2004correlation]，並且**自動處理 missing**（implicit imputation, MAR assumption）[@little2012nrcreport]
- **unstructured covariance**：不假設「兩 visit 距離越遠相關越低」，每對 visit 各有自己的 covariance；最 flexible，paper 都用這個[@mallinckrodt2004correlation; @bell2020mmrm]
:::
...
- `study + bcva_strat + lld_strat + region` — 分層因子（paper 也是這樣）；多重 prognostic factor 的 adjustment 原則見 Kahan & Morris 2013[@kahan2013adjusting]
- `us(visit | patient_id)` — 「同一個 patient 跨 visit 的 covariance 是 unstructured」；UN 在 RCT longitudinal data 是 FDA 接受的預設選擇[@mallinckrodt2004correlation; @lavange2016fda]
- **Estimand 對應**：本書 demo 採「treatment policy」估計量（如 ICH E9 R1）[@mehrotra2016estimands]
:::
